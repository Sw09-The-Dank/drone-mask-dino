...
# launch_maskdino_ddp.sh
# Wrapper to run the MaskDINO launcher across multiple nodes using torch.distributed.run
# Usage:
#   ./scripts/launch_maskdino_ddp.sh <nnodes> <nproc_per_node> <node_rank> <master_addr> <master_port> [-- launch_maskdino.py args...]
# Example (on master):
#   ./scripts/launch_maskdino_ddp.sh 2 8 0 10.0.0.1 29500 -- --train-json dataset/annotations/train.json

if [ "$#" -lt 5 ]; then
  echo "Usage: $0 <nnodes> <nproc_per_node> <node_rank> <master_addr> <master_port> [-- launch_maskdino.py args...]"
  exit 2
fi
...
# Remaining args after `--` will be forwarded to launch_maskdino.py
EXTRA_ARGS=("$@")
