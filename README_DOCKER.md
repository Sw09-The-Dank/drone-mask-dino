...
sudo docker run --gpus all --rm -it --network=host --ipc=host \
  ...
  -v "$(pwd):/workspace" -w /workspace \
  drone-maskdino:latest \
  /bin/bash -lc "bash ./scripts/launch_ddp.sh 2 1 0 169.254.18.231 29500 -- --epochs 30 --ims-per-batch 32 --base-lr 0.00001 --num-workers 10 --no-resume"
```
...
sudo docker run --gpus all --rm -it --network=host --ipc=host \
  ...
  -v "$(pwd):/workspace" -w /workspace \
  drone-maskdino:latest \
  /bin/bash -lc "bash ./scripts/launch_ddp.sh 2 1 1 169.254.18.231 29500 -- --epochs 30 --ims-per-batch 32 --base-lr 0.00001 --num-workers 10 --no-resume"
```